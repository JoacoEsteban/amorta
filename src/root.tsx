...
export const AppRoot = ({ initialRouteState, siteUrl }: AppRootProps) => {
  ...
  useEffect(() => {
    const resolvedRouteState = parseRouteState(new URL(window.location.href), "resolved");
  ...
  const appNode = match(routeState)
    ...
    .with({ kind: "result", decoded: { kind: "pending" } }, (resolvedRouteState) => (
      <App
        kind="calculator"
        routeState={resolvedRouteState}
        store={sharedStore}
    ...
    .with({ kind: "result", decoded: { kind: "valid" } }, (resolvedRouteState) => (
      <App
        kind="calculator"
        routeState={resolvedRouteState}
        store={sharedStore}
